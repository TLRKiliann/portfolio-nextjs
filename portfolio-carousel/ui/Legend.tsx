...
export default function Legend() {
  const [binaryNumbers, setBinaryNumbers] = useState<BinaryNumber[]>([]);
  ...
  // Effets parallax avec plus d'amplitude
  const y1 = useTransform(scrollYProgress, [0, 1], [0, -250]);  // Grille de fond (lente)
  const y2 = useTransform(scrollYProgress, [0, 1], [0, -400]); // Particules (moyenne)
  const y3 = useTransform(scrollYProgress, [0, 1], [0, -550]); // Contenu (rapide)
  
  // Effets supplémentaires pour la profondeur
  const opacity1 = useTransform(scrollYProgress, [0, 0.5, 1], [0.2, 0.6, 0.2]);
  const scale1 = useTransform(scrollYProgress, [0, 1], [1, 1.15]);
  const blur = useTransform(scrollYProgress, [0, 1], [0, 2]);

  return (
    <section ref={ref} className="relative py-16 sm:py-34 bg-black overflow-hidden">
      {/* Grille cyberpunk animée avec effet de profondeur et parallax */}
      <motion.div 
      ...
      {/* Particules numériques flottantes avec effet parallax */}
      <motion.div 
      ...
      <div className="relative z-10 px-4 sm:px-0">
        ...
        >
          ...
          {/* Citation principale avec effet terminal */}
